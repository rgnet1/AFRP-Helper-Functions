...
(function (global) {
    ...
    const CORNER_MARGIN_KEYS = ['top_left', 'top_right', 'bottom_left', 'bottom_right'];
    ...
    function recommendedCornerMargins(canvasWidth, canvasHeight) {
        const horizontal = Math.round(BASE_RECOMMENDED_MARGIN * canvasWidth / BASE_CANVAS.width);
        const vertical = Math.round(BASE_RECOMMENDED_MARGIN * canvasHeight / BASE_CANVAS.height);
    ...
    function recommendedMargins(canvasWidth, canvasHeight) {
        const corners = recommendedCornerMargins(canvasWidth, canvasHeight);
        return {
    ...
    function scaleElementLayout(layout, fromWh, toWh) {
        ...
        if (out.corner_margins) {
            const scaledCorners = {};
            CORNER_MARGIN_KEYS.forEach(key => {
                const corner = out.corner_margins[key] || {};
                scaledCorners[key] = {
                    horizontal: scaleNum(corner.horizontal ?? BASE_RECOMMENDED_MARGIN, sx),
                    vertical: scaleNum(corner.vertical ?? BASE_RECOMMENDED_MARGIN, sy),
                };
            });
            out.corner_margins = scaledCorners;
        }
        if (out.margins) {
            out.margins = {
        ...
        Object.entries(out).forEach(([key, spec]) => {
            if (key === 'margins' || key === 'corner_margins' || !spec || typeof spec !== 'object') return;
            if (key === 'subevents') {
